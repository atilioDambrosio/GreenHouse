...
class WindTCPHandler(socketserver.BaseRequestHandler):
    """
    ...
    def handle(self):
        while True:
            self.wind = self.server.wind
            answer_recv = rec_answer(self.request).replace('"', "")
            answers = answer_recv.split(",")
            for answer in answers:
                try:
                    if "GET_" in answer:
                        answer = self.wind.dict_functions_beam[answer]()
                    elif "SET_" in answer:
                        request, value = answer.split(" ")
                        self.wind.dict_functions_beam[request](data=value)
                        answer = "23"
                    else:
