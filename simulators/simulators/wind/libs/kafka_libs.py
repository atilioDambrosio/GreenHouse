...
def read_last_message_tp(consumer=None, tp=None):
    offsets = consumer.end_offsets([tp])
    if offsets[tp] == 1 or offsets[tp] == 0:
        return "Not available"
    else:
        try:
            consumer.seek(tp, offsets[tp] - 1)
            messages = consumer.poll(timeout_ms=1000)
            value = messages[tp][-1].value
        except:
            value = 'eRRor: empty topic'
        return value

def set_topics_partitions(topic,  num_partitions):
    try:
        create_topics([topic], num_partitions)
    except Exception as e:
        print(e)
        try:
            add_partitions([topic], num_partitions)
        except Exception as e:
            print(e)


def add_partitions(topics, number_partitions):
    admin_k = get_admin_client()
    try:
        for topic in topics:
            dict_topic = {topic: NewPartitions(number_partitions)}
            admin_k.create_partitions(dict_topic)
    except Exception as e:
        print(str(e))